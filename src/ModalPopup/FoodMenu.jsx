...
const FoodMenu = ({ showMenuList, setShowMenuList }) => {
  return (
    <section
      className={`${
        showMenuList
    ...
    >
      <div className="lg:w-[400px] relative w-full mx-5 p-5 rounded bg-white shadow-xl">
        ...
        {menu_list.map((item, index) => (
          <div key={index} className="flex gap-5 hover:bg-slate-100 transition-all duration-300 ease-linear cursor-pointer items-center mb-3">
            <img
              src={item.menu_img}
              alt={item.menu_name}
              className="w-12 h-12"
